...
import com.ismet.usbterminal.decodeToPeriodicResponse
...
class MainViewModel @Inject constructor(
    ...
    private fun checkAppSettings(allowToCreateSettingFiles: Boolean, path: String? = null) {
        ...
        if (path == null || path == MEASUREMENT) {
            ...
            try {
                ...
                val content = measurementFilePath.readTextEnhanced(file).split(
                    BUTTON_FILE_FORMAT_DELIMITER
                )
        ...
        if (corruptedFiles.isNotEmpty()) {
            events.trySend(
                MainEvent.ShowCorruptionDialog(
                    "Files ${
                        corruptedFiles.joinToString(
                            separator = ", "
                        )
                    } are corrupted. Please, fix them..."
                )
            )
        } else {
    ...
    private fun waitForCooling(accessorySettings: AccessorySettings) = viewModelScope.launch(writeDispatcher) {
        events.send(
            MainEvent.ShowWaitForCoolingDialog(
                message = """
        ...
        when {
            ...
            !powerProperties.value!!.isActivated && isPowerWaitForLastResponse -> {
                ...
                if (onResponses.contains(response)) {
                    ...
                    events.trySend(
                        MainEvent.ShowToast(
                            message = "Wrong response: Got - \"$response\".Expected - ${
                                onResponses.joinToString(
                                    separator = " or "
                                ) { "\"$it\"" }
                            }"
                        )
