...
public class FileWriterThread implements Runnable {
	...
	public void run() {
		...
		try {
			File dir = new File(Environment.getExternalStorageDirectory(), dirName);//"AEToCLogs"
			if (!dir.exists()) {
				dir.mkdirs();
			}

			dir = new File(Environment.getExternalStorageDirectory() + "/" + dirName, subDirname);
			if (!dir.exists()) {
				dir.mkdirs();
			}
			...
			new PrintStream(fos).print(strT + "," + co2 + "\n");
			fos.close();
		} catch (Exception e) {
