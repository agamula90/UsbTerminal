...
public abstract class XYChart extends AbstractChart {
                    ...
                    for (Double location : yTextLabelLocations) {
                        if (minY[i] <= location && location <= maxY[i]) {
                            ...
                            if (or == Orientation.HORIZONTAL) {
                                ...
                                canvas.drawLine(right - getLabelLinePos(axisAlign), yLabel,
                                        right, yLabel, paint);
            ...
            if (showLabels) {
                ...
                if (or == Orientation.HORIZONTAL) {
                    drawText(
                            ...
                            mRenderer.getXTitle(),
                            x + width / 2,
                            bottom + mRenderer.getLabelsTextSize() * 4 / 3 + mRenderer
                                    .getXLabelsPadding() + size,
                            paint, 0);
                    for (int i = 0; i < maxScaleNumber; i++) {
                        Align axisAlign = mRenderer.getYAxisAlign(i);
                        if (axisAlign == Align.LEFT) {
                    ...
                    paint.setTextSize(mRenderer.getChartTitleTextSize());
                    drawText(canvas, mRenderer.getChartTitle(), x + size, top + height / 2,
                            paint, 0);
                }
            }
        }
    ...
    protected void drawChartValuesText(Canvas canvas, XYSeries series, XYSeriesRenderer renderer,
                                       Paint paint, List<Float> points, int seriesIndex, int
                                               startIndex) {
                ...
                if (k == 2) { // decide whether to display first two points' values or
                    ...
                    if (Math.abs(points.get(2) - points.get(0)) > renderer
                        ...
                        drawText(canvas, getLabel(renderer.getChartValuesFormat(), series.getY
                                (startIndex)),
                                points.get(0), points.get(1) - renderer.getChartValuesSpacing(),
                                paint, 0);
                        // second point
                        drawText(canvas,
                                getLabel(renderer.getChartValuesFormat(), series.getY(startIndex
                                        + 1)),
                                points.get(2), points.get(3) - renderer.getChartValuesSpacing(),
                                paint, 0);
                    ...
                    if (Math.abs(points.get(k) - previousPointX) > renderer
                        ...
                        drawText(canvas,
                                getLabel(renderer.getChartValuesFormat(), series.getY(startIndex
                                        + k / 2)),
                                points.get(k), points.get(k + 1) - renderer.getChartValuesSpacing
                                        (), paint, 0);
    ...
    protected void drawXLabels(List<Double> xLabels, Double[] xTextLabelLocations, Canvas canvas,
                               Paint paint, int left, int top, int bottom, double xPixelsPerUnit,
    ...
    protected void drawYLabels(Map<Integer, List<Double>> allYLabels, Canvas canvas, Paint paint,
                               int maxScaleNumber, int left, int right, int bottom, double[]
                                       yPixelsPerUnit, double[] minY) {
        Orientation or = mRenderer.getOrientation();
        boolean showGridX = mRenderer.isShowGridX();
        boolean showLabels = mRenderer.isShowLabels();
        for (int i = 0; i < maxScaleNumber; i++) {
                ...
                if (or == Orientation.HORIZONTAL) {
                    ...
                    if (showLabels && !textLabel) {
                        ...
                        drawText(canvas, getLabel(mRenderer.getLabelFormat(), label),
                                ...
                                yLabel - mRenderer.getYLabelsVerticalPadding(), paint, mRenderer
                                        .getYLabelsAngle());
                    }
                    if (showGridX) {
                        paint.setColor(mRenderer.getGridColor(i));
    ...
    protected void drawXTextLabels(Double[] xTextLabelLocations, Canvas canvas, Paint paint,
                                   boolean showLabels, int left, int top, int bottom, double
                                           xPixelsPerUnit, double minX,
                                   double maxX) {
        boolean showCustomTextGridX = mRenderer.isShowCustomTextGridX();
        if (showLabels) {
            paint.setColor(mRenderer.getXLabelsColor());
            for (Double location : xTextLabelLocations) {
                if (minX <= location && location <= maxX) {
                    ...
                    paint.setColor(mRenderer.getXLabelsColor());
                    canvas
                            .drawLine(xLabel, bottom, xLabel, bottom + mRenderer
                                    .getLabelsTextSize() / 3, paint);
                    drawText(canvas, mRenderer.getXTextLabel(location), xLabel,
                            bottom + mRenderer.getLabelsTextSize() * 4 / 3, paint, mRenderer
                                    .getXLabelsAngle());
