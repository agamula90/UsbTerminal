...
public class BarChart extends XYChart {
    ...
    protected ClickableArea[] clickableAreasForPoints(List<Float> points, List<Double> values,
                                                      float yAxisValue, int seriesIndex, int
                                                                  startIndex) {
    ...
    private void drawBar(Canvas canvas, float xMin, float yMin, float xMax, float yMax, int scale,
        ...
        if (renderer.isGradientEnabled()) {
            ...
            canvas
                    .drawRect(Math.round(xMin), Math.round(yMin), Math.round(xMax), Math.round
                            (yMax), paint);
        }
    }
        ...
        for (int i = 0; i < length; i += 2) {
            ...
            if (!isNullValue(value)) {
                ...
                if (value >= 0) {
                    ...
                    drawText(canvas, getLabel(renderer.getChartValuesFormat(), value), x, points
                            .get(i + 1)
                            + renderer.getChartValuesTextSize() + renderer.getChartValuesSpacing
                            () - 3, paint, 0);
                }
            }
        }
