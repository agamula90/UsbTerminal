...
public class TimeChart extends LineChart {
	...
	protected void drawXLabels(List<Double> xLabels, Double[] xTextLabelLocations, Canvas canvas,
			...
			for (int i = 0; i < length; i++) {
				...
				if (showLabels) {
					...
					canvas.drawLine(xLabel, bottom, xLabel, bottom + mRenderer.getLabelsTextSize()
							/ 3, paint);
					drawText(canvas, format.format(new Date(label)), xLabel, bottom + mRenderer
							.getLabelsTextSize() * 4 / 3 + mRenderer.getXLabelsPadding(), paint,
		...
		drawXTextLabels(xTextLabelLocations, canvas, paint, true, left, top, bottom,
	...
	private DateFormat getDateFormat(double start, double end) {
		...
		if (diff > DAY && diff < 5 * DAY) {
			format = SimpleDateFormat.getDateTimeInstance(SimpleDateFormat.SHORT, SimpleDateFormat
					.SHORT);
		} else if (diff < DAY) {
	...
	protected List<Double> getXLabels(double min, double max, int count) {
		...
		if (mStartPoint == null) {
			mStartPoint = min - (min % DAY) + DAY + new Date(Math.round(min)).getTimezoneOffset()
					* 60 * 1000;
		}
		if (count > 25) {
			count = 25;
