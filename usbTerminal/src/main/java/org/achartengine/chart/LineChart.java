...
public class LineChart extends XYChart {
	...
	public void drawSeries(Canvas canvas, Paint paint, List<Float> points, XYSeriesRenderer
			renderer, float yAxisValue, int seriesIndex, int startIndex) {
			...
			if (fill.getType() != FillOutsideLine.Type.NONE) {
				...
				switch (fill.getType()) {
					...
					case ABOVE:
						...
						break;
					default:
						throw new RuntimeException("You have added a new type of filling but have " +
								"not implemented.");
				}
				if (fill.getType() == FillOutsideLine.Type.BOUNDS_ABOVE || fill.getType() ==
						FillOutsideLine.Type.BOUNDS_BELOW) {
					List<Float> boundsPoints = new ArrayList<Float>();
					boolean add = false;
					if (fill.getType() == FillOutsideLine.Type.BOUNDS_ABOVE && fillPoints.get(1) <
							referencePoint || fill.getType() == FillOutsideLine.Type.BOUNDS_BELOW
					...
					for (int i = 3; i < fillPoints.size(); i += 2) {
						...
						if (prevValue < referencePoint && value > referencePoint || prevValue >
							...
							if (fill.getType() == FillOutsideLine.Type.BOUNDS_ABOVE && value >
									referencePoint || fill.getType() == FillOutsideLine.Type
									.BOUNDS_BELOW && value < referencePoint) {
								i += 2;
								add = false;
							} else {
								boundsPoints.add(x);
							...
							if (add || fill.getType() == FillOutsideLine.Type.BOUNDS_ABOVE &&
									value < referencePoint || fill.getType() == FillOutsideLine
									.Type.BOUNDS_BELOW && value > referencePoint) {
	...
	protected ClickableArea[] clickableAreasForPoints(List<Float> points, List<Double> values,
		...
		for (int i = 0; i < length; i += 2) {
			...
			ret[i / 2] = new ClickableArea(new RectF(points.get(i) - selectableBuffer, points.get
					(i + 1) - selectableBuffer, points.get(i) + selectableBuffer, points.get(i +
					1) + selectableBuffer), values.get(i), values.get(i + 1));
		}
		return ret;
	}
