...
public class SquarePointHelper {
	...
	public static final String CREATE_REQUEST = "create table " + TABLE_NAME +
			" ( " + BaseColumns._ID + " integer primary key autoincrement, " + AvgPointHelper.ID +
			" integer not null);";
	...
	public List<Long> getSquarePointIds(long avgPointId) {
		Cursor cursor = writeDb.query(TABLE_NAME, new String[]{BaseColumns._ID}, AvgPointHelper.ID
				+ " = ?", new String[]{"" + avgPointId}, null, null, null);
	...
	private long getAvgPointId(long squarePointId) {
		Cursor cursor = writeDb.query(TABLE_NAME, new String[]{AvgPointHelper.ID}, BaseColumns._ID
				+ " = ?", new String[]{"" + squarePointId}, null, null, null);
