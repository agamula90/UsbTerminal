...
public class FileListAdapter extends ArrayAdapter<File> {
	 ...
	 */
	public FileListAdapter(final Context context, final List<File> objects, final File folder) {
	...
	public View getView(final int position, final View convertView, final ViewGroup parent) {
		...
		if (textView != null) {
			...
			if ((mSelection != null) && (mSelection.contains(file))) {
				selectedIcon = getContext().getResources().getDrawable(R.drawable.selected);
			...
			if (mIconOnTop) {
				...
				textView.setCompoundDrawables(thumbnail, null, selectedIcon, null);
	...
	protected int getIconForItem(final File file, final int position) {
		...
		if (file == null) {
			...
			if ((position == 0) && (mFolder != null) && (file.equals(mFolder.getParentFile())) &&
	...
	protected int resizeThumbnail(final Drawable thumbnail) {
		...
		if (thumbnail != null) {
			double ratio;
			ratio = ((double) thumbnail.getIntrinsicWidth()) / ((double) thumbnail
					.getIntrinsicHeight());
			if (ratio > 1) {
